...
async function test() {
  const { data: tenants, error: tenantsError } = await supabase.from('tenants').select('name, elevenlabs_api_key')
  console.log('Tenants:', tenants, tenantsError)

  const { data: agents, error: agentsError } = await supabase.from('ai_agents').select('name, audio_response_enabled, voice_id')
