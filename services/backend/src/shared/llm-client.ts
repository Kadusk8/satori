...
// Porta 1:1 de supabase/functions/_shared/llm-client.ts (Deno.env.get → process.env)
...
async function callGemini(params: {
  ...
  const body = {
    contents: [{ role: 'user', parts: [{ text: params.system }] }, ...params.messages],
...
export async function callLLM(params: LLMCallParams): Promise<LLMResponse> {
  ...
  if (provider === 'openai') {
    const messages: OpenAIMessage[] = params.messages.flatMap((m) => {
      ...
      for (const block of m.content) {
        if (block.type === 'tool_result') {
          ...
          result.push({ role: m.role, content: block.text || '' })
        }
      }
    ...
    for (const tc of result.toolCalls) content.push({ type: 'tool_use', id: tc.id, name: tc.name, input: tc.input })
      ...
      if (typeof m.content === 'string') {
        ...
        for (const block of m.content) {
          if (block.type === 'text') {
            ...
            parts.push({ functionResponse: { name: block.tool_use_id || 'unknown', response: { result: block.content || '' } } })
          }
        }
      }
